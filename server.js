        ...
        if (salasActivas[salaID]) {
            ...
            salasActivas[salaID].intervaloMuerte = setInterval(() => {
                if (salasActivas[salaID]) {
                    let sala = salasActivas[salaID];
                    if (sala.isBotRoom) {
                        //Evita que la vida se siga restando cuando alguien pierde
                        if (sala.bot.salud <= 0 || sala.saludJugador <= 0) return;
                        ...
                        //Para las partidas de los jugadores
                        if (sala.salud1 <= 0 || sala.salud2 <= 0) return;
...
function iniciarBotLoop(salaID, dif) {
    ...
    function botTurn() {
        ...
        //Hace que el bot deje de atacar cuando el jugador ya no tiene vida
        if (!s || !s.isBotRoom || s.bot.salud <= 0 || s.saludJugador <= 0) return;
